				...
				<ul class="max-h-[320px] overflow-auto">
					...
					>
						<div class="flex items-center justify-between w-full hover:text-[#1C1C21] leading-[16.41px]">
							...
							/> -->
							<div class="flex gap-1 !min-w-[200px] py-2">
								<input type="checkbox" :checked="checked.indexOf(item.value) > -1" @change="handleCheck(item.value)" />
								{{ item.title }}
							</div>
						</div>
					</li>
		...
		methods: {
			...
			handleCheck(id) {
				...
				if (this.checked.indexOf(id) > -1) {
					this.checked = this.checked.filter(cId => cId != id);
				} else {
