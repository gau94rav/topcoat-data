<template>
  <div
    ...
    class="grid gap-x-8"
    :class="isList ? 'grid-cols-4' : 'grid-cols-2'"
  >
    <div
      v-if="label"
    ...
    >
      ...
      >
        <div class="flex items-center gap-1 bg-[#e4e3e8] text-[#555463] p-1 px-2 rounded-sm">
            <b v-if="!hideTagKey">{{ key }}: </b>
            <div v-for="tItem of items[key]" :key="tItem">
                {{ tItem }}
            </div>
        </div>
    ...
    <div
      ...
      :class="[
        isList && 'col-span-3 flex-col',
        !areItemsCondensed && 'gap-x-1',
      ]"
    >
      <div
      ...
      >
        {{ item
        }}<span v-if="index < items.length - 1 && !isList">{{
          seperator
        }}</span>
      </div>
      <span v-if="truncatedItemsCount" class="text-[#A2A1AF] py-2 text-sm">
      ...
      if (this.hasTags) {
        ...
        if (this.visibleItemsCount > 0 && totalCount > this.visibleItemsCount) {
          items = Object.fromEntries(
            Object.entries(items).slice(0, this.visibleItemsCount)
          );
        ...
        if (
          ...
          items.length > this.visibleItemsCount
        ) {
