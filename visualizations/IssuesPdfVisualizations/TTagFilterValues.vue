<template>
    ...
    <div v-else-if="Object.keys(items).length">
        <t-grids v-if="isList" column-count="4" gap-x="8">
            <div v-if="label" class="font-semibold text-base whitespace-nowrap flex-wrap h-max">{{ label }}</div>
            <div
                class="relative flex flex-col items-start text-base gap-x-1 whitespace-nowrap flex-wrap w-full col-span-3"
        ...
        <t-grids v-else column-count="2" gap-x="8">
            <div v-if="label" class="font-semibold text-base whitespace-nowrap flex-wrap h-max">{{ label }}</div>
            <div
                class="relative flex flex-col justify-start items-start text-base gap-1 whitespace-nowrap flex-wrap"
                :style="{ color: textColor }"
            >
                <div
                    v-for="(key, index) in Object.keys(items)"
